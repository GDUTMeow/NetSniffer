...
class fileHandler:
    ...
    def handle(
        self, raw: bytes | None = None, link: str | None = None, extension: str = 'bin'
    ):
        if not raw and not link:
            raise ParamsNotSatisfiedError(
                'Either raw data or a link must be provided to handle the file.'
            )
        if raw:
